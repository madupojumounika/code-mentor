...
const Dashboard = () => {
  ...
  const neonCard =
    "relative rounded-3xl p-[1px] bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 w-full";
  const neonInner =
    "relative h-full rounded-3xl p-6 bg-white dark:bg-gradient-to-br dark:from-gray-900 dark:to-gray-800 border border-gray-200 dark:border-white/10 shadow-xl dark:shadow-[0_0_30px_rgba(99,102,241,0.25)] hover:shadow-[0_0_60px_rgba(168,85,247,0.45)] transition-all duration-300";

  return (
    <div className="min-h-screen px-4 sm:px-6 lg:px-10 py-10 bg-gradient-to-br from-indigo-100 via-white to-purple-100 dark:from-black dark:via-gray-950 dark:to-gray-900 transition-colors duration-500 overflow-x-hidden">
      ...
      <div className="flex flex-wrap gap-6 mb-16 justify-center">
        {stats.map((stat, idx) => (
          <div key={idx} className="flex-1 min-w-[250px] max-w-sm">
                  ...
                  recentActivity.map((item, idx) => (
                    <li key={idx} className="flex items-center gap-3">
                      ...
                      {`Problem ${item.problemId} → ${
                        item.status === "Accepted" ? "Solved" : "Attempted"
                      }`}
                    </li>
                  ))
                ) : (
